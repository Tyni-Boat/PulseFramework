...
#include "PulseTweenBPActionFloat.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FTweenUpdateFloatOutputPin, float, Value);

UCLASS()
class PULSEGAMEFRAMEWORK_API UPulseTweenBPActionFloat : public UPulseTweenBPActionBase
{
	GENERATED_BODY()
public:
	float Start;
	...
	static UPulseTweenBPActionFloat* TweenFloat(float Start = 0.0f, float End = 1.0f, float DurationSecs = 1.0f,
		EPulseEase EaseType = EPulseEase::InOutQuad, float EaseParam1 = 0, float EaseParam2 = 0, float Delay = 0, int Loops = 0,
	...
	static UPulseTweenBPActionFloat* TweenFloatCustomCurve(float Start = 0.0f, float End = 1.0f, float DurationSecs = 1.0f,
