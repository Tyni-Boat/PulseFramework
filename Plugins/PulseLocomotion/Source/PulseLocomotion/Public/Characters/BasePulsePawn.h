...
class PULSELOCOMOTION_API ABasePulsePawn : public APawn
{
	GENERATED_BODY()
...
protected:
	...
	FVector CurrentBalance = FVector(0, 1, 0);

	// The balance the current balance will automatically try to recover to 
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Balance")
	FVector TargetBalance = FVector(0, 1, 0);

	// last frame balance state
	EPulseLocomotionBalanceState _lastBalanceState = PerfectlyBalanced;
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Balance")
	FVector NormalBalanceRecovery = FVector(2, 1, 1);
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Balance")
	FVector CriticalBalanceRecovery = FVector(8, 0.5, 1);
